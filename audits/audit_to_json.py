...
# --- Metadata Parsing Functions (Modified) ---
...
def parse_spec_block(spec_text):
    ...
    # 'profile' has been added back to this list as requested.
...
def parse_ui_metadata_for_file(full_content):
    ...
    # Extract display_name, spec, and variables
...
# --- Individual Policy Check Parsing Functions (Unchanged) ---
...
# --- Main Application Logic (Modified for one-folder-per-file) ---
def main():
    if len(sys.argv) < 2:
        print("Usage: python final.py <file1.audit> <file2.audit> ...")
        sys.exit(1)

    # Process each file into its own separate folder
    for file_path in sys.argv[1:]:
        # Create a folder name from the audit file's base name
        output_folder = os.path.splitext(os.path.basename(file_path))[0]
        os.makedirs(output_folder, exist_ok=True)
        print(f"\n--- Processing: {os.path.basename(file_path)} ---")
        print(f"Output folder: '{output_folder}/'")

        # --- 1. Generate metadata.json for this file ---
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                content = f.read()
        ...
        if metadata:
            ...
            print(f"  - Could not parse metadata for {os.path.basename(file_path)}.")

        # --- 2. Generate individual policy JSONs for this file ---
        process_policy_checks(file_path, output_folder)
